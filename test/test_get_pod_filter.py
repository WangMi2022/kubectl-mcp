...
def test_get_pod_filter(pod_name, base_url="http://localhost:8000", match_mode="exact"):
    """
    测试 get_pod_filter 工具
    ...
    Args:
        ...
        match_mode: 匹配模式 (exact=精确匹配, fuzzy=模糊匹配)
    """
    print(f"=" * 60)
    print(f"测试 get_pod_filter 工具")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print(f"用法: python test_get_pod_filter.py <pod_name>\n")
    ...
    # 测试查询 Pod
    print("\n")
    success = test_get_pod_filter(pod_name, base_url)
    ...
    if success:
        ...
        print("=" * 60)
        # 提取 Pod 名称的前缀进行模糊匹配
        prefix = pod_name.split("-")[0] if "-" in pod_name else pod_name[:5]
        print(f"使用前缀 '{prefix}' 进行模糊匹配...")
        test_get_pod_filter(prefix, base_url, match_mode="fuzzy")
